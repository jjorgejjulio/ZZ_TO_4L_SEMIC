...
def mass_zz(row):
    ...
    if  abs(row['PID1']) == abs(row['PID2']) and abs(row['PID1']) != abs(row['PID3']) and row['Q1']!=row['Q2']:
    ...
    elif row['Q1'] != row['Q2'] and row['Q1'] != row['Q3'] and abs(row['PID1']) == abs(row['PID2']) and abs(row['PID1']) == abs(row['PID3']) and b2<103:
        if dpt1<dpt2 and dm1<dm2 and 12.15 < a2 < 103.5:
           return (v1+v2).mass , (v3+v4).mass
        else:
             return (v1+v3).mass , (v2+v4).mass    
    elif row['Q1'] != row['Q2'] and row['Q1'] != row['Q4'] and abs(row['PID1']) == abs(row['PID2']) and abs(row['PID1']) == abs(row['PID4']) and c2 < 103 :
        if dpt1<dpt3 and dm1<dm3 and 12.15 < a2 < 103.5:
           return (v1+v2).mass , (v3+v4).mass
        else:
        ...
        if dpt2<dpt3 and dm2<dm3 and 12.15 < b2 < 103.5:
           return (v1+v3).mass , (v2+v4).mass
        else:
             return (v1+v4).mass , (v2+v3).mass
